...
public interface IUiBuilder
{
    ...
    void AddHotkey(string id, string label, string defaultKey);
    void AddIntInput(string id, string label, int defaultValue, int step = 1, int stepFast = 10);
    void AddLabel(string id, string text);

    void AddQtHotkey(string label, IHotkeyResolver resolver);
    void AddQtToggle(string id, string label, bool defaultValue, string? tooltip = null, string? color = null);
